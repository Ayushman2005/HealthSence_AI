...
import { 
  Info, User, CheckCircle, AlertCircle, Lock, AlertTriangle 
} from 'lucide-react';
    ...
    <div className="space-y-8 animate-fade-in no-print text-slate-100">
      ...
      <div className="glass-panel rounded-3xl p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 border border-amber-500/20 shadow-xl">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h3 className="font-extrabold text-xl text-white tracking-tight">{userProfile?.name || 'Anonymous User'}</h3>
            <p className="text-xs text-slate-400 mt-1 flex items-center gap-2">
              ...
              {userProfile?.created_at && userProfile.created_at !== 'System Default' ? (
                <span className="text-slate-400">• Joined {new Date(userProfile.created_at).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric' })}</span>
              ) : (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div className="glass-panel rounded-3xl p-6 md:p-8 flex flex-col justify-between border border-amber-500/20 shadow-xl">
          <div>
            <div className="flex items-center gap-2.5 mb-6">
              ...
              <h3 className="font-extrabold text-lg text-white">Update Profile Information</h3>
            </div>
        ...
        <div className="glass-panel rounded-3xl p-6 md:p-8 border border-amber-500/20 shadow-xl">
          <div className="flex items-center gap-2.5 mb-6">
            ...
            <h3 className="font-extrabold text-lg text-white">Change Password</h3>
          </div>
